            ...
            return (
                <figure className="mb-6">
                    {block._localFile
                        ...
                        : <SmartImage src={block.url} alt={block.caption || ""} className="w-full rounded-xl object-cover max-h-96" />
                    }
                ...
                <figure className="mb-6">
                    <div className="aspect-video rounded-xl overflow-hidden bg-black">
                        {info.source === "google-drive"
                            ? <iframe src={`https://drive.google.com/file/d/${info.fileId}/preview`} className="w-full h-full" allowFullScreen allow="autoplay" title={block.caption || "فيديو"} style={{ border: "none" }} />
                            : block._localFile
        ...
        case "audio":
            return (
                <figure className="mb-6 bg-gray-50 dark:bg-stone-800 rounded-xl p-4">
                    {info.source === "google-drive"
                        ? <iframe src={`https://drive.google.com/file/d/${info.fileId}/preview`} className="w-full h-20" style={{ border: "none" }} allow="autoplay" title={block.caption || "صوت"} />
...
function CommentsSection({ articleId }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        if (!form.name.trim()) return setError("يرجى إدخال الاسم")
        if (!form.email.trim() || !form.email.includes("@")) return setError("يرجى إدخال بريد إلكتروني صحيح")
        if (!form.body.trim()) return setError("يرجى كتابة تعليقك")
    ...
    return (
        ...
        <span className="text-xl font-black text-stone-900 dark:text-white">
          التعليقات
            {comments.length > 0 && (
                <span className="mr-2 text-sm font-bold text-gray-400">({comments.length})</span>
            )}
        </span>
            ...
            <div className="bg-gray-50 dark:bg-stone-800 rounded-xl p-6 mb-8 border border-gray-200 dark:border-stone-700">
                <h3 className="text-sm font-bold text-stone-800 dark:text-white mb-4">أضف تعليقك</h3>

                {submitted && (
                    <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
                                className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 text-sm rounded-lg mb-4">
                ...
                {error && (
                    <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
                                className="bg-red-50 border border-red-200 text-red-600 px-4 py-3 text-sm rounded-lg mb-4">
                        {error}
                    </motion.div>
                )}
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1.5">الاسم *</label>
                    ...
                    <div>
                        <label className="block text-xs font-bold text-gray-500 mb-1.5">التعليق *</label>
                    ...
                    <div className="flex items-center justify-between">
                        <p className="text-xs text-gray-400">البريد الإلكتروني لن يظهر للعموم</p>
                        <button type="submit"
                                className="bg-amber-500 hover:bg-amber-400 text-black font-bold px-8 py-2.5 text-sm rounded-lg transition-colors">
                            إرسال التعليق ←
                        </button>
                    </div>
                </form>
            ...
            {comments.length === 0 ? (
                <div className="text-center py-10 text-gray-400">
                    ...
                    <p className="text-sm">كن أول من يعلق على هذا المقال</p>
                </div>
            ) : (
                    ...
                    {comments.map((comment) => (
                        <motion.div key={comment.id}
                            ...
                            <div className="flex items-start gap-4">
                                {/* Avatar */}
                                <div className="w-10 h-10 rounded-full bg-amber-100 dark:bg-amber-900 flex items-center justify-center text-amber-600 dark:text-amber-400 font-black text-lg shrink-0">
                                    {comment.name.charAt(0)}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-3 mb-1">
                                        <span className="text-sm font-bold text-stone-800 dark:text-white">{comment.name}</span>
    ...
    if (!article) {
        return (
            <div className="text-center py-20 text-gray-400" dir="rtl">
                ...
                <p className="text-lg font-bold">المقال غير موجود</p>
        ...
        <main className="max-w-4xl mx-auto px-4 py-10" dir="rtl">
          ...
          <span className={`text-xs font-bold px-3 py-1 rounded-full ${categoryColors[article.category] || "bg-gray-100 text-gray-700"}`}>
            {article.category}
                ...
                <h1 className="text-4xl font-black text-stone-900 dark:text-white mt-2 mb-4 leading-snug">
                    {article.title}
                </h1>

                {article.excerpt && (
                    <p className="text-lg text-gray-500 dark:text-gray-400 leading-relaxed mb-6 border-r-4 border-amber-400 pr-4">
                        {article.excerpt}
                    </p>
                )}
            ...
            {/* Cover Image */}
            {article.image && (
                <motion.div className="mb-10 rounded-xl overflow-hidden article-img"
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.3 }}>
                ...
                {hasOldBody && (
                    <p className="text-stone-700 dark:text-stone-300 leading-loose text-base whitespace-pre-line mb-12">
                        {article.body}
                    </p>
                )}
            </motion.div>
            ...
            {related.length > 0 && (
                <div className="mt-12">
                    <div className="flex items-center gap-4 mb-6">
                        <span className="text-lg font-black text-stone-900 dark:text-white">مقالات ذات صلة</span>
                        <div className="flex-1 h-px bg-gray-200 dark:bg-stone-700"></div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {related.map((rel) => (
                            <div key={rel.id} className="cursor-pointer group" onClick={() => navigate(`/article/${rel.id}`)}>
                                ...
                                <h3 className="text-sm font-bold text-stone-800 dark:text-gray-200 group-hover:text-amber-600 transition-colors leading-snug">
                                    {rel.title}
                                </h3>
                            </div>
                        ))}
