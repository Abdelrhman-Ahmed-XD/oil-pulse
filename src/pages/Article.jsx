...
function RenderBlock({ block }) {
    ...
    const safeUrl = block.url?.trim() || null
        ...
        case "image":
            return (
                <figure className="mb-6">
                    {block._localFile
                        ? <img src={safeUrl} alt={block.caption || ""} className="w-full rounded-xl object-cover max-h-96" />
                        : <SmartImage src={safeUrl} alt={block.caption || ""} className="w-full rounded-xl object-cover max-h-96" />
                    }
                ...
                <figure className="mb-6">
                    <div className="aspect-video rounded-xl overflow-hidden bg-black">
                        {info.source === "google-drive"
                            ? <iframe src={`https://drive.google.com/file/d/${info.fileId}/preview`} className="w-full h-full" allowFullScreen allow="autoplay" title={block.caption || "Video"} style={{ border: "none" }} />
                            : block._localFile
                                ? <video controls className="w-full h-full"><source src={safeUrl} /></video>
        ...
        case "audio":
            return (
                <figure className="mb-6 bg-gray-50 dark:bg-stone-800 rounded-xl p-4">
                    {info.source === "google-drive"
                        ? <iframe src={`https://drive.google.com/file/d/${info.fileId}/preview`} className="w-full h-20" style={{ border: "none" }} allow="autoplay" title={block.caption || "Audio"} />
                        : <audio controls className="w-full"><source src={safeUrl} /></audio>
                    }
        ...
        <div className="mt-16">
            <div className="flex items-center gap-4 mb-8">
                <span className="text-xl font-black text-stone-900 dark:text-white">
                    {t("comments")} {comments.length > 0 && <span className="mx-2 text-sm font-bold text-gray-400">({comments.length})</span>}
                </span>
                <div className="flex-1 h-px bg-gray-200 dark:bg-stone-700"></div>
            </div>
            <div className="bg-gray-50 dark:bg-stone-800 rounded-xl p-6 mb-8 border border-gray-200 dark:border-stone-700">
                <h3 className="text-sm font-bold text-stone-800 dark:text-white mb-4">{t("add_comment")}</h3>
                {submitted && <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 text-sm rounded-lg mb-4">{t("comment_success")}</motion.div>}
                {error && <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="bg-red-50 border border-red-200 text-red-600 px-4 py-3 text-sm rounded-lg mb-4">{error}</motion.div>}
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1.5">{t("name_label")}</label>
                            <input type="text" value={form.name} onChange={(e) => { setForm({ ...form, name: e.target.value }); setError("") }} className="w-full border border-gray-300 dark:border-stone-600 dark:bg-stone-700 dark:text-white px-4 py-2.5 text-sm outline-none focus:border-amber-400 rounded-lg" placeholder={t("name_placeholder")} />
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1.5">{t("email_label")}</label>
                            <input type="email" value={form.email} onChange={(e) => { setForm({ ...form, email: e.target.value }); setError("") }} className="w-full border border-gray-300 dark:border-stone-600 dark:bg-stone-700 dark:text-white px-4 py-2.5 text-sm outline-none focus:border-amber-400 rounded-lg" placeholder={t("email_placeholder")} />
                        </div>
                    </div>
                    <div>
                        <label className="block text-xs font-bold text-gray-500 mb-1.5">{t("comment_label")}</label>
                        <textarea value={form.body} onChange={(e) => { setForm({ ...form, body: e.target.value }); setError("") }} rows={4} className="w-full border border-gray-300 dark:border-stone-600 dark:bg-stone-700 dark:text-white px-4 py-2.5 text-sm outline-none focus:border-amber-400 rounded-lg resize-none" placeholder={t("comment_placeholder")} />
                    </div>
                    <div className="flex items-center justify-between">
                        <p className="text-xs text-gray-400">{t("email_wont_be_published")}</p>
                        <button type="submit" className="bg-amber-500 hover:bg-amber-400 text-black font-bold px-8 py-2.5 text-sm rounded-lg transition-colors">{t("send_comment")}</button>
                    </div>
                </form>
            </div>
            {comments.length === 0 ? (
                <div className="text-center py-10 text-gray-400"><p className="text-3xl mb-2">💬</p><p className="text-sm">{t("be_first_comment")}</p></div>
            ) : (
                <div className="space-y-4">
                    {comments.map((comment) => (
                        <motion.div key={comment.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-stone-800 border border-gray-200 dark:border-stone-700 rounded-xl p-5">
                            <div className="flex items-start gap-4">
                                <div className="w-10 h-10 rounded-full bg-amber-100 dark:bg-amber-900 flex items-center justify-center text-amber-600 dark:text-amber-400 font-black text-lg shrink-0">{comment.name.charAt(0)}</div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-3 mb-1"><span className="text-sm font-bold text-stone-800 dark:text-white">{comment.name}</span><span className="text-xs text-gray-400">{comment.date}</span></div>
...
export default function Article() {
    ...
    if (!article) {
        return (
            <div className="text-center py-20 text-gray-400" dir={isRtl ? "rtl" : "ltr"}>
                ...
                <button onClick={() => navigate("/")} className="mt-6 text-amber-600 font-bold text-sm hover:text-amber-500 transition-colors">
                    {t("back_to_home")}
                </button>
            </div>
        )
    ...
    return (
        <main className="max-w-4xl mx-auto px-4 py-10" dir={isRtl ? "rtl" : "ltr"}>
            ...
            >
                {isRtl ? "← " + t("back_to") + t(backName) : t("back_to") + t(backName) + " →"}
            </button>
            ...
            {article.image && (
                <motion.div className="mb-10 rounded-xl overflow-hidden article-img" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.2 }}>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.3 }}>
                ...
                {hasOldBody && <p className="text-stone-700 dark:text-stone-300 leading-loose text-base whitespace-pre-line mb-12">{article.body}</p>}
            </motion.div>

            {related.length > 0 && (
                <div className="mt-12">
                    <div className="flex items-center gap-4 mb-6">
                        <span className="text-lg font-black text-stone-900 dark:text-white">{t("related_articles")}</span>
                        <div className="flex-1 h-px bg-gray-200 dark:bg-stone-700"></div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {related.map((rel) => (
                            <div key={rel.id} className="cursor-pointer group" onClick={() => navigate(`/article/${rel.id}`)}>
                                <div className="overflow-hidden mb-3 rounded-xl article-img">
                                    <SmartImage src={rel.image} alt={rel.title} className="w-full h-32 object-cover group-hover:scale-105 transition-transform duration-500" />
                                </div>
