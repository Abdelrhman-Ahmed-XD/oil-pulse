...
export default function CategoriesList() {
    ...
    const save = (updated) => {
        ...
        // Also update the flat categories key for backwards compat
    ...
    return (
        <div dir="rtl">
            <h1 className="text-xl sm:text-2xl font-black text-stone-900 mb-2">إدارة التصنيفات</h1>
            <p className="text-sm text-gray-400 mb-6">اسحب لإعادة الترتيب · انقر للتوسيع وإدارة التصنيفات الفرعية</p>

            {/* Add Category */}
            <div className="bg-white p-5 sm:p-6 rounded-xl border border-gray-200 mb-6">
                <h2 className="text-xs font-black text-gray-500 tracking-widest mb-3">إضافة تصنيف رئيسي جديد</h2>
                ...
                {categories.map((cat, i) => (
                    <div key={cat.id}
                         ...
                         className={`bg-white rounded-xl border transition-all ${
                             dragIdx === i ? "border-amber-400 opacity-60" : "border-gray-200"
                         }`}>

                        {/* Category Row */}
                        <div className="flex items-center gap-3 px-4 py-3.5">
                            <span className="text-gray-300 cursor-grab active:cursor-grabbing text-base select-none shrink-0">⠿</span>
                            <span className="text-stone-400 text-sm font-bold w-5 shrink-0">{i + 1}</span>

                            {editingCat === cat.id ? (
                                <input type="text" value={editingCatName}
                                       onChange={(e) => setEditingCatName(e.target.value)}
                                       onKeyDown={(e) => e.key === "Enter" && saveEditCat(cat.id)}
                                       className="flex-1 border border-amber-400 px-3 py-1.5 text-sm outline-none rounded-lg" autoFocus />
                            ) : (
                                <span className="flex-1 font-bold text-stone-800 text-sm">{cat.name}</span>
                            )}
                            ...
                            <div className="flex items-center gap-1.5 shrink-0">
                                {editingCat === cat.id ? (
                                    <>
                                        ...
                                        <button onClick={() => setEditingCat(null)} className="text-xs border border-gray-300 text-gray-500 px-2.5 py-1 rounded-lg">✕</button>
                                    </>
                                ) : (
                                    <>
                                        <button onClick={() => startEditCat(cat)} className="text-xs border border-blue-200 text-blue-600 hover:bg-blue-500 hover:text-white px-2.5 py-1 rounded-lg transition-colors">تعديل</button>
                                        <button onClick={() => setExpandedCat(expandedCat === cat.id ? null : cat.id)}
                                                className="text-xs border border-gray-200 text-gray-600 hover:border-amber-300 px-2.5 py-1 rounded-lg transition-colors">
                                            {expandedCat === cat.id ? "إخفاء" : "فرعية"}
                                        </button>
                                        <button onClick={() => removeCategory(cat.id)} className="text-xs border border-red-200 text-red-500 hover:bg-red-500 hover:text-white px-2.5 py-1 rounded-lg transition-colors">حذف</button>
                                    </>
                                )}
                            </div>
                        </div>
                            ...
                            {expandedCat === cat.id && (
                                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
                                    ...
                                    <div className="px-4 pb-4 border-t border-gray-100 pt-3">
                                        <p className="text-xs font-bold text-gray-400 mb-3">التصنيفات الفرعية</p>

                                        {/* Sub list */}
                                        <div className="space-y-2 mb-3">
                                            {(cat.subcategories || []).length === 0 ? (
                                                <p className="text-xs text-gray-400 italic">لا توجد تصنيفات فرعية بعد</p>
                                            ) : (
                                                cat.subcategories.map((sub) => (
                                                    <div key={sub} className="flex items-center gap-3 bg-gray-50 px-3 py-2 rounded-lg">
                                                        <span className="w-1.5 h-1.5 rounded-full bg-amber-400 shrink-0"></span>
                                                        <span className="flex-1 text-sm text-stone-700">{sub}</span>
                                        ...
                                        <div className="flex gap-2">
                                            <input type="text"
                                                   ...
                                                   className="flex-1 border border-gray-300 px-3 py-2 text-xs outline-none focus:border-amber-400 rounded-lg"
                                                   placeholder="اسم التصنيف الفرعي..." />
                                            <button onClick={() => addSubcategory(cat.id)}
                                                    className="bg-stone-800 hover:bg-stone-700 text-white font-bold px-4 text-xs rounded-lg transition-colors">
                                                + إضافة
                                            </button>
                                        </div>
                                    </div>
