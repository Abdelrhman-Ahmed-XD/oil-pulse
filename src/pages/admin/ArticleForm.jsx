...
function DrivePreview({fileId, type}) {
    return (
        <div
            className="mt-2 rounded-xl overflow-hidden border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/20">
            {type === "image" ? (
            ...
            <div
                className="px-3 py-2 flex items-center gap-2 text-xs text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 border-t border-blue-100 dark:border-blue-800">
                <span>☁️</span>
...
function UrlStatus({status}) {
    ...
    return (
        <motion.div initial={{opacity: 0, y: -4}} animate={{opacity: 1, y: 0}}
            ...
            {status === "error" && <>
                <span>❌</span><span>تعذّر تحميل الملف — تأكد أن الرابط صحيح والملف مشارك للعموم</span></>}
        </motion.div>
    )
}
    ...
    return (
        <div>
            <div className="flex gap-2">
                ...
                {!applied
                    ? <button type="button" onClick={apply}
                              className="bg-amber-500 hover:bg-amber-400 text-black font-bold px-4 text-xs rounded-lg transition-colors whitespace-nowrap">تطبيق
                        ✓</button>
                    : <button type="button" onClick={clear}
                              className="border border-red-200 dark:border-red-800 text-red-500 dark:text-red-400 hover:bg-red-500 hover:text-white px-4 text-xs rounded-lg transition-colors whitespace-nowrap">حذف
                        ✕</button>
                }
            </div>
            ...
            {applied && value && info?.source === "direct" && info?.type === "image" && (
                <><UrlStatus status={urlStatus}/>
                    <div className="mt-2 relative rounded-xl overflow-hidden">
                        ...
                        {urlStatus === "loading" && (
                            <div
                                className="absolute inset-0 bg-gray-100 dark:bg-stone-700 rounded-xl flex items-center justify-center">
            ...
            {applied && value && info?.source === "direct" && info?.type === "audio" && (
                <><UrlStatus status="success"/>
                    <div
                        className="mt-2 bg-gray-50 dark:bg-stone-700 rounded-xl p-3 border border-gray-200 dark:border-stone-600">
...
function ImageDropZone({value, onChange}) {
    ...
    if (isLocal) return (
        <div className="relative group">
            ...
            <div className="absolute top-2 right-2 bg-green-500 text-white text-xs px-2 py-1 rounded-full">✓ تم الرفع
            </div>
    ...
    return (
        <div className="space-y-3">
            <div className="flex gap-2">
                <button type="button" onClick={() => setUrlMode(false)} className={tabCls(!urlMode)}>📁 رفع / سحب
                    وإفلات
                </button>
            ...
            {!urlMode ? (
                ...
                <SmartUrlInput value={value} onChange={onChange} placeholder="الصق رابط الصورة أو Google Drive..."
                               blockType="image"/>
            )}
        </div>
    )
...
function VideoDropZone({value, onChange}) {
    ...
    const processFile = (file) => {
        if (!file.type.startsWith("video/")) {
            toast.warning("يرجى اختيار ملف فيديو")
            return
        }
        const reader = new FileReader()
        reader.onload = (e) => onChange(e.target.result)
    ...
    return (
        <div className="space-y-3">
            ...
            {!urlMode ? (
                ...
                <SmartUrlInput value={value} onChange={onChange} placeholder="رابط YouTube أو Google Drive..."
                               blockType="video"/>
            )}
        </div>
    )
...
function AudioDropZone({value, onChange}) {
    ...
    const processFile = (file) => {
        if (!file.type.startsWith("audio/")) {
            toast.warning("يرجى اختيار ملف صوتي")
            return
        }
        const reader = new FileReader()
        reader.onload = (e) => onChange(e.target.result)
    ...
    if (isLocal) return (
        <div
            className="relative group bg-gray-50 dark:bg-stone-700 rounded-xl p-4 border border-gray-200 dark:border-stone-600">
            <audio controls className="w-full">
                <source src={value}/>
            </audio>
            <div className="absolute top-2 right-2 bg-green-500 text-white text-xs px-2 py-1 rounded-full">✓ تم الرفع
    ...
    return (
        <div className="space-y-3">
            ...
            {!urlMode ? (
                ...
                <SmartUrlInput value={value} onChange={onChange} placeholder="رابط الصوت أو Google Drive..."
                               blockType="audio"/>
            )}
        </div>
    )
...
function PdfDropZone({value, onChange}) {
    ...
    const processFile = (file) => {
        if (file.type !== "application/pdf") {
            toast.warning("يرجى اختيار ملف PDF")
            return
        }
        const reader = new FileReader()
        reader.onload = (e) => onChange(e.target.result)
    ...
    return (
        <div className="space-y-3">
            ...
            {!urlMode ? (
                ...
                <SmartUrlInput value={value} onChange={onChange} placeholder="رابط PDF أو Google Drive..."
                               blockType="pdf"/>
            )}
        </div>
    )
        ...
        <div className="space-y-3">
            {blocks.map((block, i) => (
                ...
                >
                    {/* Header */}
                    <div
                        className="flex items-center gap-2 px-3 py-2 border-b border-gray-200 dark:border-stone-700 bg-gray-50 dark:bg-stone-700 rounded-t-xl">
                        <span
                            className="text-gray-400 dark:text-stone-500 cursor-grab active:cursor-grabbing text-base select-none">⠿</span>
                        ...
                        <div className="flex items-center gap-1 mr-auto">
                            <button onClick={() => moveUp(i)} disabled={i === 0}
                                    className="text-gray-400 dark:text-stone-500 hover:text-stone-700 dark:hover:text-stone-200 disabled:opacity-20 px-1.5 py-0.5 text-xs hover:bg-gray-200 dark:hover:bg-stone-600 rounded">↑
                            </button>
                            <button onClick={() => moveDown(i)} disabled={i === blocks.length - 1}
                                    className="text-gray-400 dark:text-stone-500 hover:text-stone-700 dark:hover:text-stone-200 disabled:opacity-20 px-1.5 py-0.5 text-xs hover:bg-gray-200 dark:hover:bg-stone-600 rounded">↓
                            </button>
                            <button onClick={() => remove(block.id)}
                                    className="text-red-400 hover:text-white hover:bg-red-500 px-1.5 py-0.5 text-xs rounded">✕
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="p-3">
                        ...
                        {block.type === "divider" && (
                            <div
                                className="py-3 text-center text-gray-400 dark:text-stone-500 text-sm select-none border border-dashed border-gray-300 dark:border-stone-600 rounded-lg">
                                ―――――― فاصل ――――――
                            </div>
                        )}
                        {block.type === "image" && (
                            <div className="space-y-2">
                                {block._localFile && block.url ? (
                                    <div className="relative group">
                                        ...
                                        <button type="button"
                                                onClick={() => update(block.id, {url: "", _localFile: false})}
                                                className="absolute top-2 left-2 bg-red-500 text-white text-xs px-2 py-1 rounded-lg opacity-0 group-hover:opacity-100">✕
                                        </button>
                                    </div>
                                ) : (
                                    <ImageDropZone value={block.url} onChange={(url) => update(block.id, {
                                        url,
                                        _localFile: url?.startsWith("data:")
                                    })}/>
                                )}
                        ...
                        {block.type === "video" && (
                            <div className="space-y-2">
                                <VideoDropZone value={block.url} onChange={(url) => update(block.id, {
                                    url,
                                    _localFile: url?.startsWith("data:")
                                })}/>
                                <input type="text" value={block.caption}
                        ...
                        {block.type === "audio" && (
                            <div className="space-y-2">
                                <AudioDropZone value={block.url} onChange={(url) => update(block.id, {
                                    url,
                                    _localFile: url?.startsWith("data:")
                                })}/>
                                <input type="text" value={block.caption}
                        ...
                        {block.type === "pdf" && (
                            <div className="space-y-2">
                                <PdfDropZone value={block.url} onChange={(url) => update(block.id, {
                                    url,
                                    _localFile: url?.startsWith("data:")
                                })}/>
                                <input type="text" value={block.caption}
...
function PreviewModal({form, blocks, onClose}) {
    ...
    const renderBlock = (block) => {
        ...
        switch (block.type) {
            case "text":
                return <p key={block.id}
                          className="text-stone-700 dark:text-stone-300 leading-loose text-base whitespace-pre-line mb-6">{block.content}</p>
            case "divider":
                return <hr key={block.id} className="border-gray-200 dark:border-stone-700 my-8"/>
            case "image":
                return (
                    <figure key={block.id} className="mb-6">
                        {block._localFile ? <img src={block.url} alt={block.caption}
                                                 className="w-full rounded-xl object-cover max-h-96"/>
                            : fileId ? <DrivePreview fileId={fileId} type="image"/>
                                : <img src={info?.converted || block.url} alt={block.caption}
                                       className="w-full rounded-xl object-cover max-h-96"
    ...
    return (
        <motion.div initial={{opacity: 0}} animate={{opacity: 1}} exit={{opacity: 0}}
            ...
            <div
                className="sticky top-0 z-10 bg-white dark:bg-stone-900 border-b border-gray-200 dark:border-stone-700 px-6 py-3 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-3">
                    <div className="w-2 h-2 rounded-full bg-amber-400 animate-pulse"></div>
                    <span className="text-sm font-bold text-gray-600 dark:text-stone-300">معاينة المقال</span>
                </div>
                <button onClick={onClose}
                        className="text-sm font-bold text-gray-500 dark:text-stone-400 hover:text-gray-800 dark:hover:text-white border border-gray-300 dark:border-stone-600 px-4 py-2 hover:border-gray-500 rounded-lg">✕
                    إغلاق
                </button>
            </div>
            <div className="max-w-4xl mx-auto px-4 py-10">
                <div className="flex items-center gap-2 mb-4 flex-wrap">
                    <span
                        className={`text-xs font-bold px-3 py-1 rounded-full ${categoryColors[form.category] || "bg-gray-100 text-gray-700"}`}>{form.category}</span>
                    {form.subcategory && <><span className="text-gray-400 text-xs">←</span><span
                        className="text-xs font-bold px-3 py-1 rounded-full bg-gray-100 text-gray-600">{form.subcategory}</span></>}
                </div>
                <h1 className="text-4xl font-black text-stone-900 dark:text-white mb-4 leading-snug">{form.title ||
                    <span className="text-gray-300">عنوان الخبر...</span>}</h1>
                {form.excerpt &&
                    <p className="text-lg text-gray-500 leading-relaxed mb-6 border-r-4 border-amber-400 pr-4">{form.excerpt}</p>}
                <div
                    className="flex items-center gap-4 text-sm text-gray-400 mb-8 pb-8 border-b border-gray-200 dark:border-stone-700">
                ...
                {form.image && (
                    <div className="mb-10 rounded-xl overflow-hidden">
                        {coverInfo?.fileId ? <DrivePreview fileId={coverInfo.fileId} type="image"/>
                            : <img src={coverInfo?.converted || form.image} alt={form.title}
                ...
                <div>
                    {blocks.length === 0 && !form.image
                        ? <div
                            className="text-center py-16 text-gray-300 border-2 border-dashed border-gray-200 dark:border-stone-700 rounded-xl">
                            <p className="text-4xl mb-2">📝</p><p className="text-sm">لا يوجد محتوى بعد</p></div>
...
function CoverImageUpload({value, onChange}) {
    return (
        <div>
            <label className="block text-xs font-bold text-gray-600 dark:text-stone-400 mb-3 tracking-widest">صورة
                الغلاف</label>
...
export default function ArticleForm({user, isEdit = false}) {
    ...
    const handleSubmit = () => {
        if (!form.title.trim()) {
            toast.warning(t("name_required") || "عنوان الخبر مطلوب")
            return
        }
    ...
    return (
        <div dir={isRtl ? "rtl" : "ltr"}>
            ...
            <div className="space-y-5">
                ...
                <div
                    className="bg-white dark:bg-stone-800 p-6 rounded-xl border border-gray-200 dark:border-stone-700 grid grid-cols-2 gap-6">
                    <div>
                        <label
                            className="block text-xs font-bold text-gray-600 dark:text-stone-400 mb-2 tracking-widest">
                    ...
                    <div>
                        ...
                        <select value={form.subcategory || ""}
                            ...
                            <option
                                value="">{subcategories.length === 0 ? (lang === "ar" ? "لا توجد تصنيفات فرعية" : "No subcategories") : (lang === "ar" ? "بدون تصنيف فرعي" : "No subcategory")}</option>
                ...
                <div className="bg-white dark:bg-stone-800 p-6 rounded-xl border border-gray-200 dark:border-stone-700">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <span
                            className="text-xs text-gray-500 dark:text-stone-400 bg-gray-100 dark:bg-stone-700 px-2 py-0.5 rounded-full border border-gray-200 dark:border-stone-600">
                ...
                <div className="bg-white dark:bg-stone-800 p-6 rounded-xl border border-gray-200 dark:border-stone-700">
                    ...
                    <div
                        className="w-full border border-gray-200 dark:border-stone-600 bg-gray-50 dark:bg-stone-700 px-4 py-3 text-sm flex items-center justify-between rounded-lg">
                        <span className="font-semibold text-stone-700 dark:text-stone-200">{form.author}</span>
                        <span
                            className="text-xs text-gray-500 dark:text-stone-400 bg-gray-200 dark:bg-stone-600 px-2 py-0.5 rounded border border-gray-300 dark:border-stone-500">
