...
function SectionTitle({title, i = 0}) {
    return (
        <motion.div className="flex items-center gap-4 mb-4"
                    initial="hidden" animate="visible" custom={i} variants={fadeUp}>
            <span
                className="text-sm sm:text-base font-black tracking-widest text-stone-900 dark:text-white">{title}</span>
...
function PublisherBadge({article}) {
    ...
    return (
        <span
            className="inline-flex items-center gap-1 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-800 px-2 py-0.5 rounded text-xs font-bold">
...
export default function Analytics({user}) {
    ...
    allArticles.forEach((a) => {
        const pub = a.publishedBy || a.author || "غير معروف"
    ...
    return (
        <div dir={isRtl ? "rtl" : "ltr"}>
            ...
            {/* Stat Cards */}
            <div
                className={`grid grid-cols-2 gap-3 mb-8 ${user.role === "admin" ? "sm:grid-cols-4" : "sm:grid-cols-3"}`}>
                ...
                {user.role === "admin" &&
                    <StatCard label={t("avg_views")} value={avgViews} icon="📊" color="red" i={3}/>}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-10">
                ...
                <div>
                    ...
                    {topArticles.length === 0 ? (
                        <div
                            className="bg-white dark:bg-stone-800 p-8 text-center text-gray-400 text-sm rounded-xl border border-gray-200 dark:border-stone-700">{t("no_articles_yet")}</div>
                    ) : (
                        <div className="space-y-2">
                            {topArticles.map((article, i) => (
                                <motion.div key={article.id}
                                    ...
                                    <span
                                        className="text-xl sm:text-2xl font-black text-gray-200 dark:text-stone-600 w-6 sm:w-8 shrink-0">{i + 1}</span>
                                    <div className="flex-1 min-w-0">
                                        <p className="text-xs sm:text-sm font-bold text-stone-800 dark:text-stone-100 line-clamp-2">{article.title}</p>
                                        <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                                            <span
                                                className="text-xs text-gray-400 dark:text-stone-500">{article.category} · {article.date}</span>
                ...
                <div className="space-y-8">
                    ...
                    <div>
                        ...
                        {topCategories.length === 0 ? (
                            <div
                                className="bg-white dark:bg-stone-800 p-6 text-center text-gray-400 text-sm rounded-xl border border-gray-200 dark:border-stone-700">لا
                                توجد بيانات</div>
                        ) : (
                            <div className="space-y-3">
                                {topCategories.map(([cat, count], i) => (
                                    <motion.div key={cat} initial="hidden" animate="visible" custom={i + 6}
                                                variants={fadeUp}>
                                        <div className="flex items-center justify-between mb-1">
                                            <span
                                                className="text-xs sm:text-sm font-semibold text-stone-700 dark:text-stone-300">{cat}</span>
                    ...
                    <div>
                        ...
                        {latestArticles.length === 0 ? (
                            <div
                                className="bg-white dark:bg-stone-800 p-6 text-center text-gray-400 text-sm rounded-xl border border-gray-200 dark:border-stone-700">{t("no_articles_yet")}</div>
                        ) : (
                            <div className="space-y-2">
                                {latestArticles.map((article, i) => (
                                    <motion.div key={article.id}
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                                                <span
                                                    className="text-xs text-gray-400 dark:text-stone-500">{article.author} · {article.date}</span>
                                                {user.role === "admin" && <PublisherBadge article={article}/>}
                                            </div>
                                        </div>
                                        <span
                                            className="text-xs bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400 px-2 py-0.5 font-bold shrink-0 rounded">
                                            {article.category}
                                        </span>
                                    </motion.div>
                                ))}
                ...
                <div className="mt-8">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                        {publisherBreakdown.map(([publisher, count]) => (
                            <div key={publisher}
                                 className="bg-white dark:bg-stone-800 border border-gray-200 dark:border-stone-700 rounded-xl px-4 py-4 text-center">
                                <div
                                    className="w-9 h-9 rounded-full bg-blue-50 dark:bg-blue-900/30 flex items-center justify-center text-blue-600 dark:text-blue-400 font-black text-lg mx-auto mb-2">
                                ...
                                <p className="text-xs text-gray-400">مقال</p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="mt-10">
                ...
                {comments.length === 0 ? (
                    <div
                        className="bg-white dark:bg-stone-800 p-10 text-center text-gray-400 rounded-xl border border-gray-200 dark:border-stone-700">
                        ...
                        {comments.map((comment, i) => (
                            <motion.div key={`${comment.articleId}_${comment.id}`}
                                ...
                                <div className="flex items-start gap-3">
                                    <div
                                        className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center text-amber-600 dark:text-amber-400 font-black shrink-0 text-sm">
                                        {comment.name.charAt(0)}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 flex-wrap mb-1">
                                            <span
                                                className="text-sm font-bold text-stone-800 dark:text-stone-100">{comment.name}</span>
                                            <span
                                                className="text-xs text-gray-400 hidden sm:block">{comment.email}</span>
                                ...
                                <div
                                    className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100 dark:border-stone-700">
