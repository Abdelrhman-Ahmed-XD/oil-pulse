            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-10">
                ...
                <div>
                    ...
                    {topArticles.length === 0 ? (
                        <div className="bg-white dark:bg-stone-800 p-8 text-center text-gray-400 text-sm rounded-xl border border-gray-200 dark:border-stone-700">لا توجد مقالات بعد</div>
                    ) : (
                                ...
                                <motion.div key={article.id}
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                                            <span className="text-xs text-gray-400 dark:text-stone-500">{article.category} · {article.date}</span>
                ...
                <div className="space-y-8">
                    ...
                    <div>
                        ...
                        {topCategories.length === 0 ? (
                            <div className="bg-white dark:bg-stone-800 p-6 text-center text-gray-400 text-sm rounded-xl border border-gray-200 dark:border-stone-700">لا توجد بيانات</div>
                        ) : (
                            <div className="space-y-3">
                                {topCategories.map(([cat, count], i) => (
                                    <motion.div key={cat} initial="hidden" animate="visible" custom={i + 6} variants={fadeUp}>
                                        <div className="flex items-center justify-between mb-1">
                                            <span className="text-xs sm:text-sm font-semibold text-stone-700 dark:text-stone-300">{cat}</span>
                    ...
                    <div>
                        ...
                        {latestArticles.length === 0 ? (
                            <div className="bg-white dark:bg-stone-800 p-6 text-center text-gray-400 text-sm rounded-xl border border-gray-200 dark:border-stone-700">لا توجد مقالات بعد</div>
                        ) : (
                            <div className="space-y-2">
                                {latestArticles.map((article, i) => (
                                    <motion.div key={article.id}
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                                                <span className="text-xs text-gray-400 dark:text-stone-500">{article.author} · {article.date}</span>
                                                {user.role === "admin" && <PublisherBadge article={article} />}
                                            </div>
                                        </div>
                                        <span className="text-xs bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400 px-2 py-0.5 font-bold shrink-0 rounded">
                                            {article.category}
                                        </span>
                                    </motion.div>
                                ))}
