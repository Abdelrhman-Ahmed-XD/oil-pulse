...
function SectionTitle({ title, i = 0 }) {
    return (
        <motion.div className="flex items-center gap-4 mb-4"
                    initial="hidden" animate="visible" custom={i} variants={fadeUp}>
            <span className="text-sm sm:text-base font-black tracking-widest text-stone-900">{title}</span>
...
function PublisherBadge({ article }) {
    ...
    return (
        <span className="inline-flex items-center gap-1 bg-blue-50 text-blue-600 border border-blue-100 px-2 py-0.5 rounded text-xs font-bold">
        ...
        <div dir="rtl">
            <motion.div className="mb-6" initial="hidden" animate="visible" variants={fadeUp}>
                <h1 className="text-xl sm:text-2xl font-black text-stone-900">
                    {user.role === "admin" ? "لوحة الإحصاءات" : "إحصاءاتي"}
                </h1>
                <p className="text-sm text-gray-400 mt-1">
                    {user.role === "admin" ? "نظرة عامة على أداء الموقع" : `مقالاتك باسم "${user.username}"`}
                </p>
            </motion.div>
            ...
            <div className={`grid grid-cols-2 gap-3 mb-8 ${user.role === "admin" ? "sm:grid-cols-4" : "sm:grid-cols-3"}`}>
                ...
                {user.role === "admin" && <StatCard label="متوسط المشاهدات" value={avgViews} icon="📊" color="red" i={3} />}
            </div>
                        ...
                        <div className="space-y-2">
                            {topArticles.map((article, i) => (
                                <motion.div key={article.id}
                                            className="bg-white flex items-center gap-3 px-3 sm:px-4 py-3 cursor-pointer hover:border-amber-300 border border-gray-200 rounded-xl transition-colors"
                                            initial="hidden" animate="visible" custom={i + 6} variants={fadeUp}
                                            onClick={() => navigate(`/article/${article.id}`)}>
                                    <span className="text-xl sm:text-2xl font-black text-gray-200 w-6 sm:w-8 shrink-0">{i + 1}</span>
                                    <div className="flex-1 min-w-0">
                                        <p className="text-xs sm:text-sm font-bold text-stone-800 line-clamp-2">{article.title}</p>
                ...
                <div className="space-y-8">
                    ...
                    <div>
                        ...
                        {topCategories.length === 0 ? (
                            <div className="bg-white p-6 text-center text-gray-400 text-sm rounded-xl border border-gray-200">لا توجد بيانات</div>
                        ) : (
                            <div className="space-y-3">
                                {topCategories.map(([cat, count], i) => (
                                    <motion.div key={cat} initial="hidden" animate="visible" custom={i + 6} variants={fadeUp}>
                                        <div className="flex items-center justify-between mb-1">
                                            <span className="text-xs sm:text-sm font-semibold text-stone-700">{cat}</span>
                    ...
                    <div>
                        ...
                        {latestArticles.length === 0 ? (
                            <div className="bg-white p-6 text-center text-gray-400 text-sm rounded-xl border border-gray-200">لا توجد مقالات بعد</div>
                        ) : (
                            <div className="space-y-2">
                                {latestArticles.map((article, i) => (
                                    <motion.div key={article.id}
                                                ...
                                                onClick={() => navigate(`/article/${article.id}`)}>
                                        <div className="flex-1 min-w-0">
                                            <p className="text-xs sm:text-sm font-bold text-stone-800 line-clamp-1">{article.title}</p>
                                            <div className="flex items-center gap-1 mt-0.5 flex-wrap">
            ...
            {user.role === "admin" && publisherBreakdown.length > 0 && (
                <div className="mt-8">
                    <SectionTitle title="المقالات حسب الناشر" i={7} />
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                        {publisherBreakdown.map(([publisher, count]) => (
                            <div key={publisher} className="bg-white border border-gray-200 rounded-xl px-4 py-4 text-center">
                                <div className="w-9 h-9 rounded-full bg-blue-50 flex items-center justify-center text-blue-600 font-black text-lg mx-auto mb-2">
                                    {publisher.charAt(0).toUpperCase()}
                                </div>
                                <p className="text-xs sm:text-sm font-bold text-stone-800 truncate">{publisher}</p>
                        ...
                        {comments.map((comment, i) => (
                            <motion.div key={`${comment.articleId}_${comment.id}`}
                                ...
                                <div className="flex items-start gap-3">
                                    <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-amber-100 flex items-center justify-center text-amber-600 font-black shrink-0 text-sm">
                                        {comment.name.charAt(0)}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 flex-wrap mb-1">
                                            <span className="text-sm font-bold text-stone-800">{comment.name}</span>
                                            <span className="text-xs text-gray-400 hidden sm:block">{comment.email}</span>
                                            <span className="text-xs text-gray-400">{comment.date}</span>
                                        </div>
                                        <p className="text-xs sm:text-sm text-stone-600 leading-relaxed mb-2">{comment.body}</p>
                                        <p className="text-xs text-amber-600 font-semibold truncate">📰 {comment.articleTitle}</p>
                                    </div>
                                </div>
