...
export default function Header() {
    ...
    return (
        <header className="sticky top-0 z-50 bg-white dark:bg-stone-900 border-b border-gray-200 dark:border-stone-800 shadow-sm">

            {/* ── Newsbar ── */}
            <div className="bg-stone-900 dark:bg-black text-white py-2 overflow-hidden">
                <div className="flex items-center gap-4 px-4">
          <span className="text-amber-400 font-bold text-xs shrink-0 border border-amber-400 px-2 py-0.5 rounded">
            عاجل
          </span>
                    <div className="overflow-hidden w-full">
                        <motion.div
                            ...
                            animate={{ x: ["0%", "-50%"] }}
                            transition={{ duration: Math.max(20, newsbarTexts.length * 8), repeat: Infinity, ease: "linear" }}>
            ...
            <span className="text-xl font-black tracking-widest text-stone-900 dark:text-white leading-tight group-hover:text-amber-600 transition-colors duration-200">
              نفط <span className="text-amber-500">وطاقة</span>
                        ...
                        <span className="text-xs text-gray-400 tracking-widest leading-tight">OIL & ENERGY</span>
                    </div>
                </Link>

                {/* Desktop Nav */}
                <nav className="hidden md:flex items-center gap-6 flex-1 justify-center" dir="rtl">
                    <NavLink to="/" active={pathname === "/"}>الرئيسية</NavLink>
                            ...
                            <div key={cat.id} className="relative"
                                ...
                                <div className="relative group flex flex-col items-center">
                                    <Link to={`/category/${slug}`}
                                        ...
                                        {hasSubs && (
                                            <motion.span
                                                className="text-xs opacity-40 mr-0.5"
                                ...
                                <AnimatePresence>
                                    {hasSubs && openDropdown === cat.id && (
                                        <motion.div
                                            ...
                                            className="absolute top-full right-0 mt-3 bg-white dark:bg-stone-800 border border-gray-200 dark:border-stone-700 shadow-xl min-w-48 z-50 rounded-xl overflow-hidden"
                                            dir="rtl">
                ...
                <div className="flex items-center gap-3 shrink-0">
                    ...
                    <button
                        ...
                        aria-label="القائمة">
                        <motion.span animate={{ rotate: menuOpen ? 45 : 0, y: menuOpen ? 8 : 0 }} className="block w-5 h-0.5 bg-current rounded-full origin-center" />
                        <motion.span animate={{ opacity: menuOpen ? 0 : 1 }} className="block w-5 h-0.5 bg-current rounded-full" />
                        <motion.span animate={{ rotate: menuOpen ? -45 : 0, y: menuOpen ? -8 : 0 }} className="block w-5 h-0.5 bg-current rounded-full origin-center" />
                    </button>
                </div>
            </div>
                    ...
                    <motion.div
                        ...
                        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.05 }}>
                            <Link to="/" onClick={() => setMenuOpen(false)}
                                ...
                                {pathname === "/" && <span className="w-1.5 h-1.5 rounded-full bg-amber-500 shrink-0"></span>}
                                الرئيسية
                            </Link>
                        </motion.div>
                        ...
                        {navCategories.map((cat, idx) => {
                            ...
                            return (
                                <motion.div key={cat.id}
                                    ...
                                    <div className={`flex items-center border-b border-gray-100 dark:border-stone-800 transition-colors ${
                                        ...
                                        <Link to={`/category/${slug}`} onClick={() => setMenuOpen(false)}
                                            ...
                                            {isActive(slug) && <span className="w-1.5 h-1.5 rounded-full bg-amber-500 shrink-0"></span>}
                                            {cat.name}
                                        </Link>
                                        {hasSubs && (
                                            <button
                                                ...
                                                <motion.span animate={{ rotate: isExpanded ? 180 : 0 }} className="block text-xs">▾</motion.span>
                                            </button>
                                        )}
                                    </div>
                        ...
                        <div className="px-6 py-4 flex items-center justify-between bg-gray-50 dark:bg-stone-800">
                            <span className="text-xs text-gray-400">الوضع الداكن</span>
                            <DarkToggle dark={dark} toggle={toggle} />
                        </div>
                    </motion.div>
                )}
